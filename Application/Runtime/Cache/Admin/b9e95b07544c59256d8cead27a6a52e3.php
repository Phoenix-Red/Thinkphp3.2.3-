...
$(document).ready(function(){
	$("#loginout").click(function(){
						...
						<table class="table table-bordered">
							...
							<tbody>
								<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="long-td">
                                        ...
                                        <td style="text-align:left"><?php echo (mb_substr(strip_tags($vo["content"]),0,25,'utf-8')); ?>...</td>
                                        <td><?php echo (date('Y年m月d日 H:i:s',$vo["add_time"])); ?></td>
                                        <td><?php echo ($vo["ip"]); ?></td>
                                        <td>
                                        	<?php if($vo["c_content"] != null): ?><span class="label label-primary">已回复</span>
                                        		<?php else: ?>
		...
		function del(id,p){
			...
			layer.close(index);
			window.location.href="/Admin/Liuyan/del/id/"+id+"/p/"+p+"";
			});
		}
		</script>
