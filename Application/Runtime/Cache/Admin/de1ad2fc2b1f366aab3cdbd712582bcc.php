...
$(document).ready(function(){
	$("#loginout").click(function(){
						...
						<ul class="list-group">
							<li class="list-group-item">
								<p class="text-success"><a href="<?php echo U('Index/index');?>" title="返回首页" class="tip-bottom"><i class="fa fa-home"></i> 首页</a> /
									<a href="#" class="tip-bottom"> 系统管理</a> / <a href="<?php echo U('System/indexNotice');?>" class="tip-bottom">系统消息</a></p>
							</li>
						</ul>
					</div>
				</div>
		...
		<a href="<?php echo U('System/addNotice');?>" class="btn btn-primary">添加消息</a>
		<br />
		<br />
		<div class="row">
			<div class="col-sm-12">
				<div class="ibox float-e-margins">
					<div class="ibox-title">
						<h5><i class="fa fa-tasks"></i> 系统消息</h5>
					</div>
						...
						<table class="table table-bordered">
							...
							<tbody>
								<?php if(is_array($list)): $val = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($val % 2 );++$val;?><tr class="long-td">
										...
										<td style="text-align: center"><?php echo (stripslashes(htmlspecialchars_decode($vo["info"]))); ?></td>
										<td style="text-align: center"><?php echo (date('Y-m-d H:i:s',$vo["add_time"])); ?></td>
										<td style="text-align: center">
											<a href="<?php echo U('editNotice',array('id'=>$vo['id']));?>" class="btn btn-primary btn-mini"><i class="fa fa-pencil"></i> 编辑</a>&nbsp;&nbsp;
											<a onClick="return confirm('是否删除此条记录')" href="<?php echo U('delNotice',array('id'=>$vo['id']));?>" class="btn btn-danger btn-mini"><i class="fa fa-trash-o"></i> 删除</a>
										</td>
